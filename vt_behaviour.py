...
logging.basicConfig(filename="vt_behaviour.log", format="%(asctime)s %(name)s %(levelname)s: %(message)s", level=logging.DEBUG)
...
target_dir = input("Enter name of target directory: ")

curr_dir = os.path.dirname(os.path.abspath(__file__))
new_dir = os.path.join(curr_dir, target_dir)
pathlib.Path(new_dir).mkdir(parents=True, exist_ok=True)

all_hashes = pd.DataFrame(os.listdir(dir_name), columns=["sample"])  # Load batch file names.
submitted_files = pd.read_csv("submitted_files.csv")  # Load all file names and ids.

# Get all ids that exist in batch.
...
current_analyses = selected_analyses.loc[~selected_analyses.isin(existing_results["analysis id"])]

for idx, curr_id in enumerate(current_analyses.values):
    ...
    save_path = os.path.join(new_dir, f"{curr_id}")
